        ...
        <div style={{ padding:"18px 16px 0" }}>
          ...
          {(p.star5||p.star4||p.star3||p.star2||p.star1) && (
            <div style={{ marginBottom:16 }}>
              ...
              {[[5,p.star5],[4,p.star4],[3,p.star3],[2,p.star2],[1,p.star1]].map(([s,c])=>(
                <div key={s} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
                  ...
                  <div style={{ flex:1, height:7, background:"#e5e7eb", borderRadius:4 }}><div style={{ height:7, borderRadius:4, background:s>=4?"#f59e0b":s===3?"#f97316":"#ef4444", width:`${Math.max(4,100-(5-s)*18)}%` }} /></div>
                  <span style={{ fontSize:12, color:"#6b7280", width:48, textAlign:"right" }}>{c||"—"}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ marginBottom:8 }}>
              {[[5,p.star5],[4,p.star4],[3,p.star3],[2,p.star2],[1,p.star1]].map(([s,c])=>(
                <div key={s} style={{ display:"flex", alignItems:"center", gap:4, marginBottom:2 }}>
                  ...
                  <span style={{ fontSize:9, color:"#6b7280", width:30, textAlign:"right" }}>{c||""}</span>
                </div>
              ))}
            </div>
          )}
...
function SignInModal({ onClose, onSignIn }) {
  ...
  const [isSignUp, setIsSignUp] = useState(false); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  const handleAuth = async () => {
    setError(""); setLoading(true);
    try {
      const r = await fetch(`${SB_URL}/auth/v1/${isSignUp?"signup":"token?grant_type=password"}`, { method:"POST", headers:{ apikey:SB_KEY, "Content-Type":"application/json" }, body:JSON.stringify({ email, password }) });
      const data = await r.json();
      if (data.error||data.error_code) throw new Error(data.error_description||"Auth failed");
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:1000, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:500, padding:"24px 20px 32px" }}>
        ...
        <h2 style={{ fontSize:22, fontWeight:800, textAlign:"center", margin:"0 0 4px" }}>{isSignUp?"Create Account":"Welcome Back"}</h2>
...
function AdminGate({ onExit }) {
  ...
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      const check = await fetch(`${SB_URL}/rest/v1/admins?email=eq.${encodeURIComponent(email)}&select=email`, {
        headers:{ apikey:SB_KEY, Authorization:`Bearer ${data.access_token}` }
      }).then(r=>r.json());
      if (!Array.isArray(check)||check.length===0) throw new Error("Aap admin nahi hain. Access denied! ❌");
      setAdminUser(data.user||{ email });
    } catch(e) { setError(e.message); }
    setLoading(false);
  };

  if (adminUser) return <AdminPanel onExit={onExit} />;

  return (
    <div style={{ fontFamily:"'Segoe UI',sans-serif", minHeight:"100vh", background:"linear-gradient(135deg,#6d28d9,#9333ea,#db2777)", display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:"#fff", borderRadius:24, padding:"32px 24px", width:"100%", maxWidth:380 }}>
        <div style={{ textAlign:"center", marginBottom:24 }}>
          <img src={LOGO_URL} alt="Supplix" style={{ height:50, width:"auto", marginBottom:12 }} />
          <h2 style={{ fontSize:22, fontWeight:900, color:"#111", margin:"0 0 4px" }}>Admin Login</h2>
          <p style={{ fontSize:13, color:"#9ca3af", margin:0 }}>Sirf authorized admins access kar sakte hain</p>
        </div>
        <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Admin Email" style={{ width:"100%", border:"1.5px solid #e5e7eb", borderRadius:12, padding:"12px 14px", fontSize:14, marginBottom:10, outline:"none", boxSizing:"border-box" }} />
        <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password" onKeyDown={e=>e.key==="Enter"&&handleAdminLogin()} style={{ width:"100%", border:"1.5px solid #e5e7eb", borderRadius:12, padding:"12px 14px", fontSize:14, marginBottom:14, outline:"none", boxSizing:"border-box" }} />
        {error && <div style={{ background:"#fef2f2", border:"1px solid #fecaca", borderRadius:12, padding:"10px 14px", fontSize:13, color:"#dc2626", marginBottom:12 }}>{error}</div>}
        <button onClick={handleAdminLogin} disabled={loading} style={{ width:"100%", background:loading?"#e5e7eb":"linear-gradient(135deg,#6d28d9,#db2777)", color:loading?"#9ca3af":"#fff", border:"none", borderRadius:12, padding:"14px 0", fontSize:15, fontWeight:800, cursor:loading?"not-allowed":"pointer", marginBottom:12, opacity:loading?0.7:1 }}>
          {loading?"Checking...":"🔐 Login"}
        </button>
        <button onClick={onExit} style={{ width:"100%", background:"#f3f4f6", color:"#374151", border:"none", borderRadius:12, padding:"12px 0", fontSize:14, fontWeight:600, cursor:"pointer" }}>← Wapis Site Pe</button>
      </div>
    </div>
  );
}
...
function AdminPanel({ onExit }) {
  ...
  const FIXED = {
    trending: { label:"Trending", icon:"📈", color:"#2563eb", table:"products", tabFilter:"tab=eq.trending", extra:{ tab:"trending" },
      fields:[
        { key:"section_headline", label:"Section Headline", type:"text", placeholder:"e.g. Trending Sanitary Products on Meesho" },
    ...
    growing: { label:"Growing", icon:"⚡", color:"#0d9488", table:"products", tabFilter:"tab=eq.growing", extra:{ tab:"growing" },
      fields:[
        { key:"section_headline", label:"Section Headline", type:"text", placeholder:"e.g. Growing Products on Meesho" },
        { key:"name", label:"Product Name *", type:"textarea", required:true },
        { key:"price", label:"Price (₹)", type:"number" },
        ...
        { key:"sold_count", label:"Sold Count", type:"text" },
        { key:"star5", label:"5★ Count", type:"text" },
        { key:"star4", label:"4★ Count", type:"text" },
        { key:"star3", label:"3★ Count", type:"text" },
        { key:"star2", label:"2★ Count", type:"text" },
    ...
    suppliers: { label:"Suppliers", icon:"👥", color:"#7c3aed", table:"suppliers", tabFilter:"", extra:{},
      fields:[
    ...
    brands: { label:"Brands", icon:"🏪", color:"#0d9488", table:"brands", tabFilter:"", extra:{},
      fields:[
  ...
  const activeColor = isFixed ? sec?.color : "#6d28d9";

  const CAT_FIELDS = [
    { key:"section_headline", label:"Section Headline", type:"text", placeholder:"e.g. Best Electronics on Meesho" },
    { key:"name", label:"Product Name *", type:"textarea", required:true },
    { key:"price", label:"Price (₹)", type:"number" },
    { key:"rating", label:"Rating (0-5)", type:"number", step:0.1 },
    { key:"reviews", label:"Reviews Count", type:"text" },
  ...
  const handleSubmit = async () => {
    ...
    if (imgFile) {
      ...
      if (!image_url) { setMsg({text:"❌ Image upload fail.",ok:false}); setLoading(false); return; }
    }
    let extra = isFixed ? sec.extra : { tab:form.tab_type||"trending", category:section };
    const payload = { ...extra, ...form, image_url };
    delete payload.tab_type;
    ["price","rating","rank","products","best_products"].forEach(k=>{ if(payload[k]!=""&&payload[k]!==undefined) payload[k]=Number(payload[k]); else delete payload[k]; });
    ...
    } else { setMsg({text:`❌ ${result?.message||result?.details||"Error aaya"}`,ok:false}); }
    setLoading(false);
  };

  const handleDelete = async (id, tbl) => {
    if (!window.confirm("Delete karna chahte ho?")) return;
    await sbDelete(tbl||"products", id); setMsg({text:"🗑 Delete ho gaya!",ok:true}); await loadItems();
  };
      ...
      <div style={{ padding:16, maxWidth:640, margin:"0 auto" }}>
        {section==="categories" && (
          <div>
            ...
            {mainCats.length===0 ? (
              <div style={{ textAlign:"center", padding:"30px 0", color:"#9ca3af", background:"#fff", borderRadius:18 }}><div style={{ fontSize:40, marginBottom:8 }}>🗂</div><p style={{ margin:0 }}>Koi category nahi — upar se banao!</p></div>
            ) : mainCats.map(cat=>{
                ...
                <div key={cat.id} style={{ background:"#fff", borderRadius:18, padding:16, marginBottom:12, boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      <div><p style={{ margin:0, fontSize:15, fontWeight:800, color:"#111" }}>{cat.name}</p><p style={{ margin:0, fontSize:11, color:"#9ca3af" }}>{catSubs.length} subcategories</p></div>
                    </div>
                    ...
                    <div style={{ display:"flex", gap:8, overflowX:"auto", scrollbarWidth:"none", marginBottom:12, paddingBottom:4 }}>
                      {catSubs.map(sub=>(
                        <div key={sub.id} style={{ display:"flex", alignItems:"center", gap:6, background:"#f3f4f6", borderRadius:20, padding:"6px 12px", flexShrink:0 }}>
                          ...
                          <button onClick={()=>deleteCat(sub.id)} style={{ background:"none", border:"none", color:"#9ca3af", fontSize:14, cursor:"pointer", padding:0 }}>×</button>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {(isFixed||isCatTab) && (
          <>
            <div style={{ background:"#fff", borderRadius:18, padding:18, marginBottom:16, boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
              <h3 style={{ margin:"0 0 16px", fontSize:16, fontWeight:900, color:"#111" }}>➕ Naya Product Add Karo</h3>
              <div style={{ marginBottom:16 }}>
              ...
              {isCatTab && (
                <>
                  <div style={{ marginBottom:12 }}>
                    <label style={{ fontSize:13, fontWeight:700, color:"#374151", display:"block", marginBottom:5 }}>Tab Type</label>
                    <select value={form.tab_type||"trending"} onChange={e=>setForm(p=>({...p,tab_type:e.target.value}))} style={{ width:"100%", border:"1.5px solid #e5e7eb", borderRadius:12, padding:"10px 12px", fontSize:14, outline:"none", background:"#fff" }}>
                  ...
                  <div style={{ marginBottom:12 }}>
                    ...
                    <select value={form.subcategory||""} onChange={e=>setForm(p=>({...p,subcategory:e.target.value}))} style={{ width:"100%", border:"1.5px solid #e5e7eb", borderRadius:12, padding:"10px 12px", fontSize:14, outline:"none", background:"#fff" }}>
              ...
              {activeFields.map(f=>(
                <div key={f.key} style={{ marginBottom:12 }}>
                  ...
                  {f.type==="color" ? (
                    ...
                    <input type={f.type||"text"} step={f.step} value={form[f.key]||""} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))} placeholder={f.placeholder||f.label} style={{ width:"100%", border:"1.5px solid #e5e7eb", borderRadius:12, padding:"10px 12px", fontSize:14, outline:"none", boxSizing:"border-box" }} />
                  )}
                </div>
              ))}
                  ...
                  <div key={item.id} style={{ display:"flex", alignItems:"center", gap:10, background:"#f9fafb", borderRadius:14, padding:"10px 12px", border:"1px solid #f0f0f0" }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                        ...
                        {item.category && <span style={{ fontSize:11, background:"#f3f4f6", color:"#374151", padding:"2px 7px", borderRadius:8 }}>{item.category}</span>}
...
export default function App() {
  ...
  const [catStep, setCatStep] = useState("main");
  ...
  useEffect(()=>{
    const h=()=>setPage(window.location.hash==="#admin"?"admin":"home");
  ...
  if (page==="admin") return <AdminGate onExit={()=>{ window.location.hash=""; setPage("home"); }} />;

  // ── GROUP BY CATEGORY (ek category = ek row) ──
  const groupByCategory = (prods) => {
    const g = {};
    prods.forEach(p => {
      const k = p.category || "Top Products on Meesho";
      if (!g[k]) g[k] = [];
      g[k].push(p);
    });
    return g;
  ...
  const applyFilter = (prods) => {
    if (!filterCat) return prods;
    let f = prods.filter(p=>p.category===filterCat);
    if (filterSub) f = f.filter(p=>p.subcategory===filterSub);
    return f;
  };

  const TC = { trending:"#2563eb", growing:"#0d9488", suppliers:"#7c3aed", brands:"#0d9488" };
  const tc = TC[activeTab]||"#2563eb";

  const trendingGroups = groupByCategory(applyFilter(trending));
  const growingGroups  = groupByCategory(applyFilter(growing));
  ...
  const filterLabel = filterSub || filterCat;
    ...
    <div ref={scrollRef} style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#f4f4f8", minHeight:"100vh", maxWidth:430, margin:"0 auto", overflowY:"auto", overflowX:"hidden", position:"relative" }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:100, background:"#fff", padding:"12px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:"0 1px 8px rgba(0,0,0,0.07)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <button onClick={()=>{ setShowCats(true); setCatStep("main"); setSelectedMainCat(null); }} style={{ background:"none", border:"none", cursor:"pointer", padding:2 }}>
      ...
      {/* TABS */}
      <div style={{ margin:"0 16px 0", background:"#fff", borderRadius:16, padding:6, display:"flex", gap:4, boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
        {[{key:"trending",icon:"📈",label:"Trending"},{key:"growing",icon:"⚡",label:"Growing"},{key:"suppliers",icon:"👥",label:"Suppliers"},{key:"brands",icon:"🏪",label:"Brands"}].map(tab=>(
          <button key={tab.key} onClick={()=>{ setActiveTab(tab.key); setFilterCat(null); setFilterSub(null); }} style={{ flex:1, background:activeTab===tab.key?"#fff":"transparent", border:"none", borderRadius:12, padding:"8px 4px", cursor:"pointer", boxShadow:activeTab===tab.key?"0 2px 8px rgba(0,0,0,0.1)":"none" }}>
      ...
      {filterLabel && (
        <div style={{ padding:"10px 16px 0", display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6, background:"#ede9fe", borderRadius:20, padding:"5px 12px" }}>
            <span style={{ fontSize:12, fontWeight:700, color:"#6d28d9" }}>{filterLabel}</span>
            <button onClick={()=>{ setFilterCat(null); setFilterSub(null); }} style={{ background:"none", border:"none", color:"#6d28d9", fontSize:16, cursor:"pointer", padding:0, fontWeight:700 }}>×</button>
          </div>
        </div>
      )}
      ...
      <div style={{ paddingBottom:100, marginTop:12 }}>
        {loading ? (
          ...
          {/* TRENDING */}
          {activeTab==="trending" && (
            Object.keys(trendingGroups).length===0 ? <EmptyState tab="trending products" /> :
            Object.entries(trendingGroups).map(([cat, prods])=>(
              <div key={cat} style={{ marginBottom:16 }}>
                <SectionHeader icon="⚡" title={prods[0]?.section_headline || cat + " on Meesho"} total={prods.length} btnLabel="View All" btnColor={tc} />
          ...
          {/* GROWING */}
          {activeTab==="growing" && (
            Object.keys(growingGroups).length===0 ? <EmptyState tab="growing products" /> :
          ...
          {activeTab==="suppliers" && (suppliers.length===0 ? <EmptyState tab="suppliers" /> : <>
            <SectionHeader icon="👥" title="Top Suppliers on Meesho" total={suppliers.length} btnLabel="View All" btnColor={tc} />
            <div style={{ display:"flex", gap:12, overflowX:"auto", padding:"0 16px 20px", scrollbarWidth:"none" }}>{suppliers.map(s=><SupplierCard key={s.id} s={s} />)}</div>
          </>)}
          {/* BRANDS */}
          {activeTab==="brands" && (brands.length===0 ? <EmptyState tab="brands" /> : <>
            ...
            <div style={{ display:"flex", gap:12, overflowX:"auto", padding:"0 16px 20px", scrollbarWidth:"none" }}>{brands.map(b=><BrandCard key={b.id} b={b} />)}</div>
          </>)}
        </>}
      </div>
      ...
      {showCats && (
        <div onClick={()=>setShowCats(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:200, display:"flex", alignItems:"flex-end" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", width:"100%", maxWidth:430, margin:"0 auto", borderRadius:"20px 20px 0 0", maxHeight:"80vh", overflow:"hidden", display:"flex", flexDirection:"column" }}>
            <div style={{ padding:"16px 16px 12px", flexShrink:0, borderBottom:"1px solid #f0f0f0" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  {catStep==="sub" && <button onClick={()=>{ setCatStep("main"); setSelectedMainCat(null); }} style={{ background:"#f3f4f6", border:"none", borderRadius:10, width:32, height:32, cursor:"pointer", fontSize:16 }}>←</button>}
                  <span style={{ fontSize:14, fontWeight:800, color:"#111" }}>{catStep==="main"?"Categories":selectedMainCat?.name}</span>
                </div>
            ...
            <div style={{ overflowY:"auto", padding:"12px 16px 24px" }}>
              {catStep==="main" ? (
                mainCats.length===0 ? (
                  <div style={{ textAlign:"center", padding:"30px 0", color:"#9ca3af" }}><div style={{ fontSize:40, marginBottom:8 }}>🗂</div><p style={{ margin:0, fontSize:14 }}>Admin ne abhi koi category nahi banayi</p></div>
                ) : <>
                  <div onClick={()=>{ setFilterCat(null); setFilterSub(null); setShowCats(false); }} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 0", borderBottom:"1px solid #f3f4f6", cursor:"pointer" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ width:40, height:40, borderRadius:10, background:"linear-gradient(135deg,#6d28d9,#db2777)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>🛍️</div>
                      <div><p style={{ margin:0, fontSize:14, fontWeight:700, color:"#111" }}>Sab Products</p><p style={{ margin:0, fontSize:11, color:"#9ca3af" }}>Filter hatao</p></div>
                    </div>
                    {!filterCat && <span style={{ width:8, height:8, background:"#6d28d9", borderRadius:"50%" }} />}
                  </div>
                  {mainCats.map(cat=>{
                    const catSubCount = subCats.filter(s=>s.parent_id===cat.id).length;
                    return (
                      <div key={cat.id} onClick={()=>{
                        if (catSubCount>0) { setSelectedMainCat(cat); setCatStep("sub"); }
                        else { setFilterCat(cat.name); setFilterSub(null); setShowCats(false); }
                      }} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 0", borderBottom:"1px solid #f3f4f6", cursor:"pointer" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                          <div style={{ width:40, height:40, borderRadius:10, background:"#f9fafb", border:"1px solid #f0f0f0", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, fontWeight:800, color:"#6d28d9" }}>{cat.name[0].toUpperCase()}</div>
                          <div>
                            <p style={{ margin:0, fontSize:14, fontWeight:700, color:"#111" }}>{cat.name}</p>
                            <p style={{ margin:0, fontSize:11, color:"#9ca3af" }}>{catSubCount>0?`${catSubCount} subcategories`:"Tap to filter"}</p>
                          </div>
                        </div>
                        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                          {filterCat===cat.name && !filterSub && <span style={{ width:8, height:8, background:"#6d28d9", borderRadius:"50%" }} />}
                          {catSubCount>0 && <span style={{ color:"#9ca3af", fontSize:18 }}>›</span>}
                        </div>
                      </div>
                    );
                  })}
                ...
                <div onClick={()=>{ setFilterCat(selectedMainCat.name); setFilterSub(null); setShowCats(false); }} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 0", borderBottom:"1px solid #f3f4f6", cursor:"pointer" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    ...
                    <div><p style={{ margin:0, fontSize:14, fontWeight:700, color:"#111" }}>Sab {selectedMainCat?.name}</p><p style={{ margin:0, fontSize:11, color:"#9ca3af" }}>Poori category</p></div>
                  </div>
                  {filterCat===selectedMainCat?.name && !filterSub && <span style={{ width:8, height:8, background:"#6d28d9", borderRadius:"50%" }} />}
                </div>
                {subCats.filter(s=>s.parent_id===selectedMainCat?.id).map(sub=>(
                  <div key={sub.id} onClick={()=>{ setFilterCat(selectedMainCat.name); setFilterSub(sub.name); setShowCats(false); }} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 0", borderBottom:"1px solid #f3f4f6", cursor:"pointer" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <div style={{ width:40, height:40, borderRadius:10, background:"#f0fdf4", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16 }}>•</div>
                      <p style={{ margin:0, fontSize:14, fontWeight:600, color:"#111" }}>{sub.name}</p>
                    </div>
                    {filterSub===sub.name && <span style={{ width:8, height:8, background:"#6d28d9", borderRadius:"50%" }} />}
                  </div>
                ))}
              </>}
            </div>
        ...
        <div onClick={()=>setShowDrawer(false)} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.35)", zIndex:300, display:"flex" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", width:"75%", maxWidth:300, height:"100%", padding:"20px 0 0", overflowY:"auto" }}>
            ...
            <div style={{ borderTop:"1px solid #f0f0f0" }}>
              ...
              <a href="#admin" style={{ padding:"14px 20px", display:"flex", alignItems:"center", gap:10, textDecoration:"none" }}><span>⚙️</span><span style={{ fontSize:15, fontWeight:600, color:"#9ca3af" }}>Admin Panel</span></a>
            </div>
